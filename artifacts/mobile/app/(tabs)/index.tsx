...
export default function DashboardScreen() {
  ...
  const { user } = useAuth();
  const { timetable, getStudentNotifications, getStudentMidMarks } = useAppData();

  const today = DAYS[new Date().getDay() === 0 ? 6 : new Date().getDay() - 1];
  const todayClasses = timetable
    .filter((t) => t.day === today && t.batch === user?.batch)
    .sort((a, b) => a.time.localeCompare(b.time));

  const recentNotifs = getStudentNotifications(user?.batch ?? "All").slice(0, 3);
...
const styles = StyleSheet.create({
  ...
  nameText: { color: "#fff", fontSize: 28, fontFamily: "Inter_700Bold", marginBottom: 4 },
  dateText: { color: "rgba(255,255,255,0.65)", fontSize: 14, fontFamily: "Inter_400Regular" },
  statsRow: {
